...
def parse_request(req_json: dict) -> TranscriptionRequest:
    ...
    audio_bytes = bytes(req_json.get('audio_bytes'), 'utf-8') # Convert string into bytes
...
def distort(audiopath, threshold=0.6):
    ...
    distorted_samples = [int(sample * threshold) if abs(sample) < threshold * audio.max_possible_amplitude else int(audio.max_possible_amplitude if sample > 0 else -audio.max_possible_amplitude) for sample in samples]
